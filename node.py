...
class Node:
    ...
    # immediate mean if self statement node is immediate 
    # family of a given node in condiction script case
    # -1 means irrelevant node  
    def __init__(self, name, parent=None):
        ...
        self.information_dep_parents = []
        self.information_dep_children = []
    ...
    def is_real_statement(self):
        statements = ['BreakStatement', 'ContinueStatement', 'DoWhileStatement',
                      'DebuggerStatement', 'EmptyStatement', 'ExpressionStatement', 'ForStatement',
                      'ForOfStatement', 'ForInStatement', 'IfStatement', 'LabeledStatement', 'SwitchStatement', 'ThrowStatement', 'TryStatement',
                      'WhileStatement', 'WithStatement',
                      'VariableDeclaration', 'CatchClause', 'SwitchCase', 'ConditionalExpression',
                      'FunctionDeclaration', 'ClassDeclaration']
        if self.name in statements:
    ...
    def is_condition(self):
        statements = ['IfStatement', 'SwitchCase', 'ConditionalExpression', 'CatchClause', 'SwitchStatement']
        if self.name in statements:
            return True
        return False
    ...
    def is_controlflow_statement(self):
        statements = ['BreakStatement', 'ContinueStatement', 'ReturnStatement', 'TryStatement']
        if self.name in statements:
            return True
        return False
    ...
    # In terms of information flow, extremity is the another end of information edge of 
    # self node rather than statement of self node in its original purpose
    def set_information_dependency(self, extremity):
        self.information_dep_children.append(extremity)
        extremity.information_dep_parents.append(self)
